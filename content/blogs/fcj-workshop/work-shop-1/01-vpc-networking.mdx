...
- **1 Custom VPC** dải IP `10.0.0.0/16` (65,536 địa chỉ IP).
- **2 Public Subnets** rải đều ở 2 AZs (`us-east-1a`, `us-east-1b`) dành cho Application Load Balancer và Bastion Host.
- **2 Private Subnets** rải đều ở 2 AZs dành cho Web Application Server và Database Cluster (RDS).
- **1 Internet Gateway (IGW)** đóng vai trò cửa ngõ giao tiếp Internet hai chiều.
- **Bảng định tuyến (Route Tables)** được phân tách nghiêm ngặt giữa Public & Private Zone.
...
# Thêm Default Route trỏ ra IGW
aws ec2 create-route --route-table-id $PUB_RT --destination-cidr-block 0.0.0.0/0 --gateway-id $IGW_ID

# Associate Subnets vào Route Tables
...
Để chắc chắn hạ tầng mạng của bạn đã hoạt động đúng như thiết kế, hãy chạy câu lệnh kiểm tra trạng thái Route Table thông qua AWS CLI:

```bash
# Kiểm tra Route Table của Public Subnet xem đã có route 0.0.0.0/0 trỏ tới IGW chưa
...
**Kết quả mong đợi:**  
Đầu ra JSON hiển thị chi tiết `GatewayId` bắt đầu bằng `igw-xxxxxxxx` với `State: active`.
...
Hãy chuyển sang [Part 2: Compute & Database Layer](./02-compute-rds) để bắt đầu khởi tạo máy chủ EC2 và cụm cơ sở dữ liệu Amazon RDS!
