...
1. Truy cập **Amazon ECS Console** -> **Clusters** -> `neonfoodmap-cluster`.
2. Select service `neonfoodmap-backend-service` -> bấm **Update**.
...
1. Mở **Amazon SNS Console** -> **Topics** -> bấm **Create topic**.
2. Type: **Standard**, Name: `NeonFoodMap-Alerts-Topic` -> bấm **Create topic**.
3. Chọn tab **Subscriptions** -> **Create subscription**.
...
1. Mở **CloudWatch Console** -> **Alarms** -> bấm **Create alarm**.
...
5. **Alarm name:** `ECS-Backend-High-CPU-Alarm` -> Bấm **Create alarm**.
</Step>
